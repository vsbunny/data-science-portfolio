# --- Corner Plot for ObsCosNest_closed.py - Closed Universe Model --- 
import matplotlib
matplotlib.use('Agg')
...
INPUT_DATA_FOLDER = "Run_files_closed"
...
POSTERIOR_FILENAME = "posterior_closed.dat" 
...
# the CurlyM, omega_L, and omega_m parameters respectively, as defined the CPNest model.
data = np.loadtxt(data_path, skiprows=1, usecols = (0,1,2))

# True source values (Commented out in your original script)
"""
source={'t_start': 5.,
        'log_amplitude': -38.,
        'damp_tau': .01,
...
plt.savefig(os.path.join(OUTPUT_PLOT_FOLDER, 'cornerplot_closed.png'))

print(f"Corner plot saved to: {os.path.join(OUTPUT_PLOT_FOLDER, 'cornerplot_closed.png')}")
